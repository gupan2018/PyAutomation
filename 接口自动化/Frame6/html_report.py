...
class HtmlReport:
    ...
    def generate_html(self,head, file):
            ...
            #查询测试失败的用例数
            self.cursor.execute('SELECT count(case_id) FROM test_result WHERE status = %s',('0',))
            self.fail_num = self.cursor.fetchone()[0]

            #查询测试通过的用例数
            self.cursor.execute('SELECT count(case_id) FROM test_result WHERE status = %s',('1',))
            ...
            query = ('SELECT case_id, http_method, request_name, request_url,'
                          'mobilephone,regname, test_method, test_desc, status, code, msg FROM test_result')
            self.cursor.execute(query)
